...
shaka.hls.HlsParser = class {
  ...
  async updateStream_(streamInfo) {
    ...
    if (this.lowLatencyMode_) {
      let query = '_HLS_skip=YES';
      // Enable delta updates. This will replace older segments with
      // 'EXT-X-SKIP' tag in the media playlist.
      if (this.canBlockReload_ && streamInfo['nextMSN'] ) {
        query += `&_HLS_msn=${streamInfo['nextMSN']}`;
        query += `&_HLS_part=${streamInfo['nextPart']}`;
      }
      uriObj.setQueryData(new goog.Uri.QueryData(query));
    }
    ...
    // save next msn & part for blocking playlist reload
    streamInfo['nextMSN'] = mediaSequenceNumber + segments.length - 1;
    if (newestSegment.partialReferences.length > 0) {
      const preload = newestSegment.partialReferences[
          newestSegment.partialReferences.length-1].preloadHint? 1:0;
      streamInfo['nextPart'] = newestSegment.partialReferences.length - preload;
    } else {
      streamInfo['nextPart'] = 0;
    }
  ...
  determinePresentationType_(playlist) {
    ...
    if (isVod) {
      ...
      if (this.lowLatencyMode_) {
        ...
        this.canBlockReload_ = serverControlTag.getRequiredAttrValue(
